...
void loadVoxelConfig(rclcpp::Node* node, VoxelMapConfig& voxel_config) {
  ...
  // vector<int> needs specific handling if not directly supported, but rclcpp
  // usually handles std::vector<long int> Assuming default values are fine for
  // now or handle appropriately
...
void VoxelMapManager::StateEstimation(StatesGroup& state_propagat) {
  ...
  for (int iterCount = 0; iterCount < config_setting_.max_iterations_;
    ...
    if (effct_feat_num_ > 0) {
      ...
      meas_vec.setZero();
      for (size_t i = 0; i < effct_feat_num_; ++i) {
